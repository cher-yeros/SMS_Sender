...
var newD = data.map(function(record) { //loop through the excel list
    var phone = record.Phone; //reading data by attribute
    var name = record.Name;
    var sex = record.Gender;
    var nickname = record.Nickname;

    var messege = `Hi ${nickname} How are yout? I am fine.`; //messege to be sent
...
function sendIt(name, phone, messege) {
    ...
    var url = baseUrl + `?username=${uname}&password=${pwd}&phone=${phone}&message=${messege}`; //url to send SMS ; see on the app for more
