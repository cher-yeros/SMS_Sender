...
var newD = data.map(function (record) {
  var phone = record.Phone; //var name = record.Name;

  var sex = record.Gender;
  var nickname = record.Nickname; //name = name.search(" ") > 0 ? name.substring(0, name.indexOf(" ")) : name;
  ...
  var message = "Hi ".concat(nickname, " endet ").concat(greet, "? ene dena negn. How was the week? I hope arif endeneber. Zare 11:30 team endalen ").concat(last, " nw. so, Bethel Mekane Iyesus begize enegenagn. See you later. ").concat(bless, " !"); //sendIt(name, phone, messege);
...
var wsTest = xlsx.utils.json_to_sheet(horebs);
xlsx.utils.book_append_sheet(file, wsTest, "messege"); //xlsx.utils.add
// Writing to our file
...
function sendIt(name, phone, messege) {
  var uname = "yero";
  var pwd = "1234";
  var baseUrl = "http://192.168.43.46:8090/SendSMS"; //var baseUrl = "http://192.168.137.149:8090/SendSMS";
